...
LAST_BACKUP="${src}.backup.$(date +%Y%m%d%H%M%S)"
cp -a "$src" "$LAST_BACKUP"
}
...
log_error "Sistem yükseltmesi başarısız oldu."
return 1
fi
...
PACKAGES="wget curl openssl stunnel4 dropbear openssh-server python3 ufw fail2ban lsb-release uuid-runtime unzip"

for pkg in $PACKAGES; do
if ! DEBIAN_FRONTEND=noninteractive apt-get install -y "$pkg" > /dev/null 2>&1; then
log_error "$pkg paketi kurulamadı."
return 1
fi
echo -e " ${GREEN}✓${NC} $pkg"
done
...
log_error "OpenSSH yapılandırması yedeklenemedi."
return 1
fi
...
log_error "Yeni OpenSSH yapılandırması geçersiz."
...
log_error "OpenSSH servisi yeniden başlatılamadı."
return 1
fi
enable_ssh_service
...
# Portları dinamik olarak ayarla
sed -i "s/LISTEN_PORT = 80/LISTEN_PORT = $WS_PORT/" /usr/local/bin/ws-proxy.py
sed -i "s/SSH_PORT = 22/SSH_PORT = $SSH_PORT/" /usr/local/bin/ws-proxy.py
...
echo -e "${RED}Geçersiz kullanıcı adı! Sadece küçük harf, rakam, alt çizgi (_) ve tire (-) kullanılabilir. (En fazla 32 karakter, bir harf veya alt çizgi ile başlamalıdır)${NC}"
return
fi
...
if ! validate_username "$username"; then
echo -e "${RED}Geçersiz kullanıcı adı formatı!${NC}"
return
fi
...
expire=$(LC_ALL=C chage -l "$username" 2>/dev/null | grep "Account expires" | cut -d: -f2 | xargs)
...
if ! systemctl list-unit-files sshd.service >/dev/null 2>&1; then
services=("ssh" "dropbear" "stunnel4" "ws-proxy")
fi
...
echo "─────────────────────────────────────────────────────────────"
ss -tlnp 2>/dev/null | grep -E "($SSH_PORT|$WS_PORT|$DROPBEAR_PORT|$SSL_PORT|$OPENSSH_SSL_PORT)" | \
awk '{
    split($4, a, ":");
    port = a[length(a)];
    prog = $6;
...
rm -rf /etc/stunnel
rm -f /etc/sysctl.d/99-vpn-bbr.conf
sysctl --system >/dev/null 2>&1
